...
class Parser
	...
	constructor: ->
		@pos = Pos.start()
		@locals = new Locals
		E.AutoDeclares.forEach (declare) =>
	...
	justification: (tokens) ->
		...
		#rule = @get tokens[0], E.BlockElement

		#jTokenses = splitWhere (tail tokens), T.special ','
		#js = jTokenses.map (jTokens) =>
		#	@get jTokens[0], E.BlockElement

		# new E.Justification @pos, rule, js

		rule = @get tokens[0], E.Rule
		new E.Justification @pos, rule, (tail tokens).map (token) =>
	...
	logicPlain: (tokens) ->
		...
		switch tok0.constructor
			...
			when T.Name
				text = tok0.text()
				if text in [ '~', '¬' ]
					cCheck tokens.length > 1, pos, "Expected something after #{tok0}"
	...
	mustBeName: (tokens) ->
		cCheck tokens.length == 1, @pos, "Expected name, got [#{tokens}]"
		name = tokens[0]
		cCheck name instanceof T.Name, @pos, "Expected name, got #{name}"
		name.text()
	...
	rule: (tokens, proofTokens) ->
		rule = @locals.withFrame =>
			...
			newAtoms =
				(splitWhere newAtomsTokens, T.special ',').map (x) =>
					atomName = @mustBeName x
					newAtom = new E.AtomDeclare x[0].pos(), atomName
